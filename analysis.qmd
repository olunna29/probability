...
The following code simulates rolling a loaded die and visualizes the distribution of the sums. As the number of rolls increases, the distribution becomes more representative of the biased die.
```{r}
set.seed(10)
...
This code visualizes the theoretical probability distributions for both a fair and a loaded die.
```{r}
fair_probs <- tibble(
  face = 1:6,
...
This histogram shows the distribution of the first roll that crosses a 0.95 posterior probability threshold, based on 1,000 repeated experiments with the loaded die. It varies because each individual dice roll can push closer to or away from the final hypothesis, but eventually they all reach certainty.
```{r}
set.seed(10)
